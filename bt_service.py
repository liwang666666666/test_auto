class BtSevice(object):
    def __init__(self):
        self.crc_table = [
            ]

    @classmethod
    def BaleData(cls, data, length):
        head_middle_data = length | 1024
        head_middle_data = str(hex(head_middle_data)).replace('0x','')
        head_data = 'fd01' + head_middle_data[1] + head_middle_data[2] + '0' + head_middle_data[0] + '0100'
        head_data_second = head_data.encode(encoding="utf-8")
        ctc_head2 = cls().crc8(head_data_second)
        write_data = head_data + ctc_head2
        middle_data = data.hex()
        write_data = write_data + middle_data
        crc_tail2 = cls().crc8(data)
        write_data = write_data + str(crc_tail2)
        return write_data

    def crc8(self, data):
        if str(data)[2] != 'f':
            data1 = data.hex()
        else:
            data1 = str(data)[2:-1]
        a = 0
        for i in range(len(data1)):
            if i%2 == 0:
                data_2 = data1[i:i+2]
                data_2 = '0x' + data_2
                data_2 = int(data_2, base=16)
                a = data_2 ^ self.crc_table[a]
            else:
                continue
        a_data = str(hex(self.crc_table[a])).replace('0x','')
        if len(a_data) == 1:
            a_data = '0' + a_data 
        return a_data
